...
def _get_game_for_user(user_id):
    ...
    if not game:
        logger.debug("[_get_game_for_user] Stage 1 failed (chat_id=%s), trying fallback scan", chat_id)
        chat_id, game = find_game_by_active_player(redis_client, user_id)
        if game:
            logger.info("[_get_game_for_user] Fallback scan found game: user_id=%s, chat_id=%s", user_id, chat_id)
    else:
        logger.debug("[_get_game_for_user] Stage 1 success: user_id=%s, chat_id=%s", user_id, chat_id)
...
async def inline_query_handler(update, context):
    ...
    # Debug: Log game lookup result and state
    if not game:
        logger.info("[inline_query] user_id=%s: Game not found (chat_id=%s)", user_id, chat_id)
        await query.answer([], cache_time=2, is_personal=True)
        return
    
    if not game.activePlayer:
        logger.info("[inline_query] user_id=%s: activePlayer is None (phase=%s)", user_id, game.phase)
        await query.answer([], cache_time=2, is_personal=True)
        return
    
    active_player_id = game.activePlayer.id
    if active_player_id != user_id:
        logger.info("[inline_query] user_id=%s: Not active player (activePlayer=%s, phase=%s)", 
                    user_id, active_player_id, game.phase)
        await query.answer([], cache_time=2, is_personal=True)
        return
...
async def chosen_inline_result_handler(update, context):
    ...
    try:
        ...
        if success:
            ...
            logger.info("[chosen_inline_result] user_id=%s: Saved game state. activePlayer=%s, phase=%s", 
                        user_id, game.activePlayer.id if game.activePlayer else None, game.phase)
